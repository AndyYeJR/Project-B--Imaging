...
function preload() {
  ...
  bgm = loadSound('assets/Summer.mp3')
}
...
function draw() {
  ...
  //photo
  if (focus == true) {
    image(img, imgX, imgY, 410 * 3.25, 410);
  } else {
    image(imgBlur, imgX, imgY, 410 * 3.25, 410);
  }
...
function takePhoto() {
  if (mouseX > 700 && mouseX < 835 && mouseY > 165 && mouseY < 210) {
    ...
    pop()
    if (click == true) {
      saveX = map(imgX, 20, -700, 0, -1360);
      if (focus == true) {
        image(img, saveX, 0, 720 * 3.25, 720);
      } else {
        image(imgBlur, saveX, 0, 720 * 3.25, 720);
      }
